...
void cmd_help(int argc, char **argv){
    if(argc == 2){
        ...
        char path[2048];
        filename = argv[1];
        snprintf(path, sizeof(path), "src/commands/help_commands/%s.txt", filename);
        FILE* fp = fopen(path, "r");
        if(fp == NULL){
            printf("Bogo Bash:  Команда не найдена (обратитесь к справочнику 'help')\n");
            return;
        }else{
            char line[1024];
        ...
        printf(BOLD_RED "bogo - это специальный аргумент для выполнение команды с рут правами (для работы запустите Bogo Bash с рут правами)\n\n" RESET);
